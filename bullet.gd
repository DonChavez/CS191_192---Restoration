...
func _ready():
	...
	# Configure hitbox and sprite based on who fired the projectile
	if fired_by == "player":
		hitbox.set_collision_layer_value(5, true)		# Assign it as player projectile
		hitbox.set_collision_mask_value(3, true)		# Can collide with enemies
		hitbox.set_collision_mask_value(1, false)		# No collision with player
		sprite.texture = player_projectile_texture
		speed = 200									# More projecetile speed for player
	elif fired_by == "enemy":
		hitbox.set_collision_layer_value(6, true)		# Assign it as penemy projectile
		hitbox.set_collision_mask_value(1, true) 		# Can collide with player
		hitbox.set_collision_mask_value(3, false)		# No collision with other enemies
...
func _on_hit(_hurtbox: HurtboxComponent, _damage: float):
